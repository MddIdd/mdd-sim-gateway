...
def check_attachment(name: str, content_type: str, data: bytes, *,
    ...
    if declared is None and base_type(content_type) in _GENERIC_TYPES:
        declared = _BY_EXTENSION.get(extension_of(name))
...
def display_name(name: str, content_type: str = "", *, max_bytes: int = MAX_NAME_BYTES) -> str:
    """The original file name made safe to store, show and offer as a download name: path
    and control characters removed, Unicode normalised, and cut to `max_bytes` of UTF-8
    without losing the extension. An empty result becomes "attachment.<extension>"."""
...
def storage_name(seq: int, content_type: str) -> str:
    """A fresh on-disk name for a part: never derived from what a sender called the file."""
